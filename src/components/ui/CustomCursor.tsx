...
export default function CustomCursor() {
  ...
  return (
    <>
      <div
        ref={dot}
        className="fixed pointer-events-none z-[99999] -translate-x-1/2 -translate-y-1/2 w-3 h-3 rounded-full bg-yellow transition-all duration-200"
        style={{ mixBlendMode: "difference" }}
      />
      <div
        ref={ring}
        className="fixed pointer-events-none z-[99998] -translate-x-1/2 -translate-y-1/2 w-10 h-10 rounded-full border border-yellow/50 transition-all duration-300"
      />
    </>
  );
}
